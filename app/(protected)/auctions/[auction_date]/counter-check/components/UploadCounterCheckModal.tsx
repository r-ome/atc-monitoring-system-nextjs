...
import { uploadCounterCheck } from "@/app/(protected)/auctions/actions";
...
export const UploadCounterCheckModal: React.FC<UploadManifestModalProps> = ({
  auction_id,
}) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await uploadCounterCheck(auction_id, formData);

    if (res) {
      setIsLoading(false);
      if (res.ok) {
        toast.success("Successfully uploaded counter check!", {
          description:
            res.value +
